...
| 字段 | 值 |
|---|---|
| 插件名 | timestamp-toolkit ／ 时间戳转换百宝箱 |
...
- **智能解析**：ISO、RFC 2822、中文、相对日期（今天/明天/昨天）、部分日期建议、美欧月日歧义、自定义格式与正则规则；uTools 触发正则与解析引擎同规则
- **夏令时兜底**：回退重叠小时展示全部候选，跳空不存在时间明确提示
- **日历与农历**：日/月/年三级视图 + 时分秒滚轮；农历（1900–2100）与节日标注
- **节假日标注**：2024–2026 法定节假日 / 调休班（橙"休"红"班"角标）
- **顺手**：正则匹配自动转换（小数秒/时区后缀、紧凑 ISO、中文、英文月名、月日、短日期）、剪贴板预填、粘贴自动清洗、ESC 逐层回退、中英双语、三态主题
...
- 技术栈：原生 HTML/CSS/JS（11 模块按序加载）、双语 i18n（167 键）、node:test 151 项单测
- 质量：`npm run verify` 一键验收（静态校验 + 加载冒烟 + 单测）
- 版本轨迹：v1.0.0 基础转换 → v1.0.1 桌面壳/IANA/歧义修复 → v1.0.2 农历/历元加固 → v1.0.3 节假日标注 + 角标配色（首次上架）→ v1.0.4 小数秒分组显示 + 低精度导航找回 + 精度显示 UI
